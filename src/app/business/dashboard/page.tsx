...
import { getVenuePickupSlots, SLOT_CAPACITY_POINTS } from "@/lib/capacityEngine";
    ...
    <div className="min-h-screen bg-[#f7f7f7] text-[#202124]">
      ...
      <div className="lg:pl-64">
        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
          ...
          {view === "load" && <LoadView profile={profile} orders={orders} />}
...
function LoadView({ profile, orders }: { profile: BusinessProfile; orders: BusinessOrder[] }) {
  ...
  const capacitySlots = getVenuePickupSlots(profile.venue.id);
  return <div><PageTitle title="Загрузка" subtitle="Распределение поступивших заказов в течение дня" /><div className="grid gap-4 sm:grid-cols-3"><Metric icon={ShoppingBag} label="Заказов сегодня" value={total} accent="#00a082" /><Metric icon={TrendingUp} label="Пиковый интервал" value={peak.count ? peak.label : "—"} accent="#ff2d55" /><Metric icon={Clock3} label="Интервалов с заказами" value={hourly.filter((item) => item.count > 0).length} accent="#377dcc" /></div><div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_1fr]"><Panel><h2 className="text-lg font-black">Заказы по времени</h2><p className="text-sm text-[#777]">Сегодня, интервалы по часу получения</p><HourlyChart orders={orders} /></Panel><Panel><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-black">Ближайшие слоты</h2><p className="text-sm text-[#777]">QoS автоматически рассчитывает бонусы по загрузке кухни</p></div><span className="shrink-0 rounded-lg bg-[#f3f3f3] px-2.5 py-1 text-xs font-bold">Лимит {SLOT_CAPACITY_POINTS}</span></div><div className="mt-5 space-y-3">{capacitySlots.map((slot) => <div key={slot.id}><div className="mb-1.5 flex items-center gap-2 text-xs"><strong>{slot.label}</strong><span className="text-[#777]">{slot.currentLoad}/{slot.capacity} баллов</span><span className={`ml-auto font-black ${slot.bonusAmount ? "text-[#b47e00]" : "text-[#888]"}`}>{slot.available ? slot.bonusAmount ? `+${slot.bonusAmount}` : "без бонуса" : "заполнен"}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#eeeeee]"><div className={`h-full rounded-full ${slot.occupancyPercent >= 80 ? "bg-[#ff2d55]" : slot.occupancyPercent > 35 ? "bg-[#ffc244]" : "bg-[#00a082]"}`} style={{ width: `${Math.max(3, slot.occupancyPercent)}%` }} /></div></div>)}</div><p className="mt-4 text-[11px] leading-relaxed text-[#777]">Баллы загрузки — сумма сложности всех блюд в заказах на интервал. Чем выше загрузка, тем ниже бонус для следующего клиента.</p></Panel></div></div>;
}
...
function MenuView({ profile, onProfileChange }: { profile: BusinessProfile; onProfileChange: (profile: BusinessProfile) => void }) {
  const [draft, setDraft] = useState({ name: "", price: "", description: "", complexityWeight: "2" });
  ...
  const setComplexity = (id: string, complexityWeight: number) => saveMenu(profile.venue.menu.map((item) => item.id === id ? { ...item, complexityWeight } : item));
  const addItem = () => { if (!draft.name.trim() || Number(draft.price) <= 0) return; saveMenu([...profile.venue.menu, createVenueMenuItem({ name: draft.name, description: draft.description || "Фирменное блюдо", price: Number(draft.price), image: profile.venue.image, category: "Основное", complexityWeight: Number(draft.complexityWeight) })]); setDraft({ name: "", price: "", description: "", complexityWeight: "2" }); };
  const removeItem = (id: string) => saveMenu(profile.venue.menu.filter((item) => item.id !== id));
  return <div><PageTitle title="Меню" subtitle="Сложность блюд участвует в автоматическом расчёте загрузки" /><div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]"><Panel><div className="space-y-3">{profile.venue.menu.map((item) => { const unavailable = item.available === false; return <div key={item.id} className={`flex flex-wrap items-center gap-3 rounded-2xl border p-3 ${unavailable ? "border-[#eee] bg-[#f7f7f7] opacity-65" : "border-[#e5e5e5]"}`}><img src={item.image || profile.venue.image} alt={item.name} className="h-16 w-20 rounded-xl object-cover" /><div className="min-w-36 flex-1"><p className="truncate font-black">{item.name}</p><p className="text-sm text-[#777]">{item.price.toLocaleString()} ₸ · {unavailable ? "Стоп-лист" : "Доступно"}</p></div><label className="text-[11px] font-bold text-[#777]">Сложность<select value={item.complexityWeight ?? 2} onChange={(event) => setComplexity(item.id, Number(event.target.value))} className="ml-2 rounded-lg border border-[#ddd] bg-white px-2 py-1.5 text-xs font-bold text-[#222]"><option value="1">1 · лёгкая</option><option value="2">2 · обычная</option><option value="3">3 · средняя</option><option value="4">4 · сложная</option><option value="5">5 · очень сложная</option></select></label><button onClick={() => toggleItem(item)} className={`rounded-xl px-3 py-2 text-xs font-bold ${unavailable ? "bg-[#e9f8f5] text-[#007e68]" : "bg-[#fff5d6] text-[#6d5300]"}`}>{unavailable ? "Вернуть" : "Стоп"}</button><button onClick={() => removeItem(item.id)} className="rounded-full p-2 text-[#c12849]"><Trash2 className="h-4 w-4" /></button></div>; })}</div></Panel><Panel><h2 className="text-lg font-black">Новое блюдо</h2><div className="mt-4 space-y-3"><input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Название" className="business-input" /><input type="number" min="1" value={draft.price} onChange={(event) => setDraft({ ...draft, price: event.target.value })} placeholder="Цена, ₸" className="business-input" /><textarea value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="Описание" className="business-input min-h-24 resize-none" /><label className="block text-xs font-bold text-[#666]">Сложность приготовления<select value={draft.complexityWeight} onChange={(event) => setDraft({ ...draft, complexityWeight: event.target.value })} className="business-input mt-1.5"><option value="1">Лёгкая · 1 балл</option><option value="2">Обычная · 2 балла</option><option value="3">Средняя · 3 балла</option><option value="4">Сложная · 4 балла</option><option value="5">Очень сложная · 5 баллов</option></select></label><p className="text-[11px] leading-relaxed text-[#777]">Вес отражает нагрузку на кухню. QoS использует его только для расчёта доступности слотов и бонусов.</p><button onClick={addItem} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#00a082] px-4 py-3 text-sm font-black text-white"><Plus className="h-4 w-4" /> Добавить в меню</button></div></Panel></div></div>;
}
