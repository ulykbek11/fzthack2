...
export default function BusinessDashboardPage() {
  ...
  const addTestOrder = () => {
    createDemoOrder(profile);
    setView("orders");
    notify("Тестовый заказ поступил в кабинет");
  };
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 border-b border-[#e8e8e8] bg-white/95 backdrop-blur-xl">
          <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6">
            <div className="flex min-w-0 items-center gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#e9f8f5] text-[#007e68]"><Store className="h-5 w-5" /></div><div className="min-w-0"><p className="truncate font-black">{profile.venue.name}</p><p className="truncate text-xs text-[#777]">{profile.venue.address}</p></div></div>
            <div className="flex items-center gap-2"><button onClick={addTestOrder} className="hidden items-center gap-2 rounded-xl bg-[#fff5d6] px-3 py-2.5 text-xs font-bold text-[#6d5300] sm:flex"><TestTube2 className="h-4 w-4" /> Тестовый заказ</button><button onClick={toggleOrders} className={`flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-bold ${profile.acceptsOrders ? "bg-[#e9f8f5] text-[#007e68]" : "bg-[#fff0f3] text-[#bd2041]"}`}>{profile.acceptsOrders ? <PlayCircle className="h-4 w-4" /> : <PauseCircle className="h-4 w-4" />}{profile.acceptsOrders ? "Заказы включены" : "Заказы на паузе"}</button><Bell className="h-5 w-5 text-[#777]" /></div>
          </div>
        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
          {view === "overview" && <Overview profile={profile} orders={orders} activeOrders={activeOrders} newOrders={newOrders} onOpenOrders={() => setView("orders")} onAddTestOrder={addTestOrder} onChangeStatus={changeStatus} />}
          {view === "orders" && <OrdersView orders={orders} onChangeStatus={changeStatus} onAddTestOrder={addTestOrder} />}
          {view === "load" && <LoadView profile={profile} orders={orders} onProfileChange={(next) => { setProfile(next); notify("Настройки загрузки сохранены"); }} />}
          ...
          {view === "venue" && <VenueView profile={profile} onProfileChange={(next) => { setProfile(next); notify("Данные заведения сохранены"); }} onCreateAnother={() => router.push("/business")} />}
        </main>
      </div>
...
function Overview({ profile, orders, activeOrders, newOrders, onOpenOrders, onAddTestOrder, onChangeStatus }: { profile: BusinessProfile; orders: BusinessOrder[]; activeOrders: BusinessOrder[]; newOrders: number; onOpenOrders: () => void; onAddTestOrder: () => void; onChangeStatus: (id: string, status: BusinessOrderStatus) => void }) {
  ...
  return <div><PageTitle title="Обзор" subtitle="Текущая работа заведения и ближайшие заказы" /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={ListChecks} label="Новые" value={newOrders} accent="#ff2d55" /><Metric icon={ChefHat} label="Готовятся" value={orders.filter((order) => order.status === "PREPARING").length} accent="#377dcc" /><Metric icon={PackageCheck} label="Готовы к выдаче" value={ready} accent="#00a082" /><Metric icon={CircleDollarSign} label="Сумма сегодня" value={`${revenue.toLocaleString()} ₸`} accent="#c08c00" /></div><div className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]"><Panel><div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-black">Текущие заказы</h2><p className="text-sm text-[#777]">{activeOrders.length ? `${activeOrders.length} требуют внимания` : "Активных заказов пока нет"}</p></div><button onClick={onOpenOrders} className="flex items-center gap-1 text-sm font-bold text-[#007e68]">Все заказы <ChevronRight className="h-4 w-4" /></button></div>{activeOrders.length ? <div className="space-y-3">{activeOrders.slice(0, 4).map((order) => <CompactOrder key={order.id} order={order} onChangeStatus={onChangeStatus} />)}</div> : <EmptyState icon={ShoppingBag} title="Ждём первый заказ" text="Создайте тестовый заказ, чтобы проверить весь рабочий сценарий." action="Создать тестовый заказ" onAction={onAddTestOrder} />}</Panel><Panel><h2 className="text-lg font-black">Заведение сегодня</h2><div className="mt-4 space-y-3"><InfoRow label="Приём заказов" value={profile.acceptsOrders ? "Включён" : "На паузе"} good={profile.acceptsOrders} /><InfoRow label="Бонусный интервал" value={profile.venue.bonusWindow || "Не задан"} good={Boolean(profile.venue.bonusWindow)} /><InfoRow label="Блюд в меню" value={String(profile.venue.menu.filter((item) => item.price > 0).length)} good /><InfoRow label="Заказов сегодня" value={String(todayOrders.length)} good={todayOrders.length > 0} /></div><Link href="/" className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-[#f3f3f3] px-4 py-3 text-sm font-bold">Посмотреть как клиент <ExternalLink className="h-4 w-4" /></Link></Panel></div></div>;
}

function OrdersView({ orders, onChangeStatus, onAddTestOrder }: { orders: BusinessOrder[]; onChangeStatus: (id: string, status: BusinessOrderStatus) => void; onAddTestOrder: () => void }) {
  const groups: BusinessOrderStatus[] = ["NEW", "PREPARING", "READY", "PICKED_UP", "CANCELLED"];
  return <div><div className="flex flex-wrap items-end justify-between gap-3"><PageTitle title="Заказы" subtitle="Новые заказы появляются здесь автоматически" /><button onClick={onAddTestOrder} className="flex items-center gap-2 rounded-xl bg-[#fff5d6] px-4 py-3 text-sm font-bold text-[#6d5300]"><TestTube2 className="h-4 w-4" /> Создать тестовый</button></div>{orders.length === 0 ? <Panel><EmptyState icon={ShoppingBag} title="Заказов пока нет" text="Откройте заведение в клиентском каталоге или создайте тестовый заказ." action="Создать тестовый заказ" onAction={onAddTestOrder} /></Panel> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">{groups.map((status) => { const groupOrders = orders.filter((order) => order.status === status); return <div key={status}><div className="mb-3 flex items-center justify-between"><span className={`rounded-full px-3 py-1 text-xs font-black ${statusMeta[status].className}`}>{statusMeta[status].label}</span><span className="text-sm font-bold text-[#888]">{groupOrders.length}</span></div><div className="space-y-3">{groupOrders.map((order) => <OrderCard key={order.id} order={order} onChangeStatus={onChangeStatus} />)}{groupOrders.length === 0 && <div className="rounded-2xl border border-dashed border-[#ddd] p-6 text-center text-xs text-[#999]">Здесь пока пусто</div>}</div></div>; })}</div>}</div>;
}
...
function LoadView({ profile, orders, onProfileChange }: { profile: BusinessProfile; orders: BusinessOrder[]; onProfileChange: (profile: BusinessProfile) => void }) {
  const [windowValue, setWindowValue] = useState(profile.venue.bonusWindow || "15:00–17:00");
  const [amount, setAmount] = useState(String(profile.venue.bonusAmount || 200));
  const hourly = hourlyData(orders);
  const saveBonus = () => { const next = updateBusinessProfile(profile.venue.id, (current) => ({ ...current, venue: { ...current.venue, bonusWindow: windowValue, bonusAmount: Number(amount) || 0, bonuses: `+${Number(amount) || 0} в тихий час` } })); if (next) onProfileChange(next); };
  const disableBonus = () => { const next = updateBusinessProfile(profile.venue.id, (current) => ({ ...current, venue: { ...current.venue, bonusWindow: null, bonusAmount: 0, bonuses: "Без бонусов сейчас" } })); if (next) onProfileChange(next); };
  const quietest = [...hourly].sort((a, b) => a.count - b.count)[0];
  return <div><PageTitle title="Загрузка и бонусы" subtitle="Распределяйте спрос по времени дня" /><div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]"><Panel><h2 className="text-lg font-black">Заказы по времени</h2><p className="text-sm text-[#777]">Сегодня, интервалы по часу</p><HourlyChart orders={orders} /><div className="mt-5 flex items-start gap-3 rounded-2xl bg-[#e9f8f5] p-4"><Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-[#00a082]" /><p className="text-sm text-[#285b50]">Самый свободный интервал: <strong>{quietest.label}</strong>. Включите бонусы, чтобы привлечь больше предзаказов.</p></div></Panel><Panel><h2 className="text-lg font-black">Бонусный интервал</h2><p className="mt-1 text-sm text-[#777]">Отмечается в клиентском каталоге и меню.</p><label className="mt-5 block text-sm font-bold">Время<input value={windowValue} onChange={(event) => setWindowValue(event.target.value)} className="business-input mt-1.5" placeholder="15:00–17:00" /></label><label className="mt-4 block text-sm font-bold">Бонус, ₸<input type="number" min="0" value={amount} onChange={(event) => setAmount(event.target.value)} className="business-input mt-1.5" /></label><button onClick={saveBonus} className="mt-5 w-full rounded-xl bg-[#00a082] px-4 py-3 text-sm font-black text-white">Включить бонусы</button>{profile.venue.bonusWindow && <button onClick={disableBonus} className="mt-2 w-full rounded-xl px-4 py-3 text-sm font-bold text-[#c12849]">Отключить бонусы</button>}<div className="mt-5 rounded-xl bg-[#f7f7f7] p-3 text-sm"><p className="font-bold">Сейчас в каталоге</p><p className="mt-1 text-[#777]">{profile.venue.bonusWindow ? `${profile.venue.bonusWindow} · +${profile.venue.bonusAmount} бонусов` : "Бонусных интервалов нет"}</p></div></Panel></div></div>;
}
...
function VenueView({ profile, onProfileChange, onCreateAnother }: { profile: BusinessProfile; onProfileChange: (profile: BusinessProfile) => void; onCreateAnother: () => void }) {
